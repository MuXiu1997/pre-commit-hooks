...
def test_detect_pnpm_with_local_eslint(temp_project):
...
def test_detect_bun_with_local_eslint(temp_project):
...
def test_detect_deno_with_local_eslint(temp_project):
...
def test_detect_deno_remote(temp_project):
...
def test_detect_package_manager_field(temp_project):
...
def test_recursive_lookup(temp_project):
...
def test_main_execution_pnpm(mock_run, temp_project):
    ...
    mock_run.assert_called_once_with(expected_cmd, capture_output=False)

@patch('subprocess.run')
def test_main_execution_local_bun(mock_run, temp_project):
...
@patch('subprocess.run')
def test_main_error_handling(mock_run, temp_project):
